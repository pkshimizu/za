...
- **`/za:cross-ship`** — `/za:ship` と同じ通しフローだが、レビューを Claude 単独ではなく
  **Claude と Codex CLI（`codex exec`）の 2 モデル**で行う。同じレビュー観点を両方に渡して
  並行でレビューし、指摘をマージ（両方が指摘した箇所は優先度を上げる）して修正し、
  **両方が指摘なしになるまで**繰り返す。Codex だけが拾った指摘は観点ファイルの穴として
  `docs/review-perspectives/` に還元する。`codex` が使えない場合は単独レビューへ縮退せず停止する。
