...
- **`/za:cross-review`** — `/za:review` の 2 モデル版。引数で指定した PR を **Claude と Codex の
  ...
  （両方が指摘した箇所は誤検知の確率が低いので優先度を上げる）して修正し、**両方が指摘なしに
  なるまで**繰り返す。相手側だけが拾った指摘は観点ファイルの穴として
  `docs/review-perspectives/` に還元する。Claude Code から実行すると相手側は `codex exec`、
...
- **`/za:release`** — 本番リリース。プロジェクトの `docs/RELEASE.md` を読み、そこに書かれた
  「リリース前チェック」（確認コマンドと通る条件の対）を上から順にすべて実行する。1 つでも
  通らなければ、何をすれば通るかを示してリリース PR を作らずに止まる。すべて通ったらリリース
  PR の内容を提示して承認を得てから作成し、マージも別途承認を得てから行う（マージ＝本番
  デプロイ）。マージ後はデプロイのワークフローを待って各段階の結果を報告し、「リリース後の
  確認」があれば実行する。`docs/RELEASE.md` が無いプロジェクトでは止まる（ルール無しで
  リリースを走らせない）。
