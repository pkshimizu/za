...
- これらのディレクトリを用意するときは、**`INDEX.md`（1 行 1 ファイルの索引）も一緒に置く**。
  `za:fix-issue` / `za:review` は索引を先に読み、今回の実装・差分に関係するものだけを開く。
  索引が無いと全ファイルを読む挙動に縮退する（動きはするが、ファイルが増えるほど無駄が増える）。
  既にファイルが溜まっているディレクトリには `/za:index` で索引を生成できる。
- `za:release` が読む `docs/RELEASE.md` も対象外で、**ひな形も置かない**。base / head・CI の
  ワークフロー名・リリース前チェックの確認コマンドはプロジェクトごとに異なり、ひな形のままだと
  「通る条件」が意味を持たず、形だけのチェックがリリースを通してしまう。`docs/RELEASE.md` が
  無いと `za:release` は止まるので、本番リリースを za で回すときに、実際に検証したコマンドで
  プロジェクト側が書く。
